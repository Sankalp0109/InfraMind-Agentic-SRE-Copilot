...
### Embeddings: local `sentence-transformers`, not Voyage/OpenAI
The build plan suggested Voyage or OpenAI embeddings. Neither has an API key available in this environment, and rather than block Phase 2 on acquiring one, defaulted to a local `sentence-transformers` model instead — no API key, no external network dependency at retrieval time, and consistent with the project's existing lean toward staying local/dependency-light where a reasonable option exists (see Phase 1's stdlib-only tools, embedded Qdrant below). `ingest.py`/`retrieve.py` will isolate the embedding call behind one function, so swapping in a hosted provider later is a small, contained change if wanted — not a rewrite.

### Qdrant: embedded/on-disk mode, not a Docker container
This host has already demonstrably struggled under the full otel-demo stack alone (documented above: load averages of 13-14, Grafana timing out under its own weight). Adding a Qdrant *container* on top of that stack risked making an already resource-constrained dev environment worse for marginal benefit — `qdrant-client`'s embedded mode (local on-disk storage, no server process) gets the identical hybrid-retrieval behavior this project needs to demonstrate, without another container competing for CPU/memory. This is a local-dev convenience, not a production architecture choice — worth revisiting if/when this ever needs to run somewhere with real resources (e.g. Phase 6's Terraform stretch goal).

### Postmortem corpus: 18 docs, one per real flag plus 3 deliberate near-duplicates
Wrote one postmortem per verified flagd flag from Phase 0's corrected list (15 flags), each grounded in that flag's actual real behavior confirmed live in Phase 0/1 (e.g. the payment-charge-failure postmortem cites the exact exception text and stack trace line captured from a real Jaeger trace during Phase 0's flag test — not invented). Added 3 more deliberately: a second cart-failure postmortem (real dependency exhaustion, not the injected flag) and a second Kafka postmortem (broker outage vs. the flag's lag-spike scenario) both exist specifically so retrieval has two *topically adjacent but causally distinct* incidents to discriminate between; a third payment postmortem (slow-but-succeeding) rounds the payment story out to three genuinely different failure shapes for the same service. This matters for Phase 2's own stated goal — proving hybrid retrieval beats pure vector search — since that claim is only testable if the corpus contains near-duplicates a purely semantic search could plausibly confuse. Every postmortem ends with an explicit "Diagnostic signal" section stating what distinguishes it from its nearest sibling incident, both as retrieval-eval signal and as the kind of judgment call a written postmortem should capture for whoever reads it next.
